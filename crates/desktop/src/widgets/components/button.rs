...
use iced::widget::{button as btn, Row};
...
impl<'a, M: Clone + 'a> From<Button<'a, M>> for Element<'a, M> {
    fn from(value: Button<'a, M>) -> Self {
        ...
        let radius = radius.map(ButtonRadius::value).unwrap_or_else(|| size.radius());

        let mut widget = btn(inner)
...
fn palette(variant: ButtonVariant, status: btn::Status) -> (Option<Color>, Color, Color, f32) {
    ...
    match variant {
        ButtonVariant::Primary => {
            ...
            (Some(bg), color::Primary::FOREGROUND, Color::TRANSPARENT, 0.0)
        }
